...
function CurrentDraw({universe,edition,kind,onPlayer,onMatch}){
  ...
  return <section className="draw-board">{Object.entries(rounds).map(([round,matches])=><div className="draw-column" key={round}><h3>{round}</h3>{matches.map(match=>{
    ...
    return <button className="draw-match-with-portraits" key={match.id} onClick={()=>onMatch(edition.id,match.id)}><div className="draw-player-line winner">{winner&&<Portrait player={winner} size="draw"/>}<strong>{winner?fullName(winner):'Winner'}</strong></div><div className="draw-player-line">{loser&&<Portrait player={loser} size="draw"/>}<span>{loser?fullName(loser):'Opponent'}</span></div><b>{match.score}</b></button>;
  })}</div>)}</section>;
}
...
export default function TournamentDetailPage({universe,tour,name,onBack,onPlayer,onMatch,onSimulate}){
  ...
  const championPlayer=individual&&edition?.championId?findPlayer(universe,edition.championId):null;
  const majorClass=event.level==='Grand Slam'?' grand-slam-showcase':'';
  ...
  return <div className="page detail-page"><BackButton onBack={onBack} label="Back to previous page"/><header className={`tournament-hero prestige-tournament-hero surface-${String(event.surface||'hard').toLowerCase()}${majorClass}`}><div><p>{tour} · {kind==='team'?'International team competition':`${event.level}${kind==='doubles'?' doubles':''}`}</p><h1>{name}</h1><span>{event.location} · {event.dates||`Week ${event.week}`}</span></div>{kind==='team'?<span className="team-cup-mark">TEAM CUP</span>:<SurfaceBadge surface={event.surface}/>}</header><Tabs items={['Overview','Current Year','History','Stats']} active={tab} onChange={setTab}/>
  {tab==='Overview'&&<><section className="detail-stat-grid"><Stat label="Location" value={event.location}/><Stat label="Surface" value={event.surface}/><Stat label="Level" value={kind==='team'?'International':event.level}/><Stat label="Champion points" value={kind==='team'?'Team honor':championPoints(event.level).toLocaleString()}/><Stat label="Draw" value={event.drawSize||'16 countries'}/><Stat label="Week" value={event.week}/><Stat label="Defending champion" value={defendingText}/><Stat label="Status" value={edition?'Completed':'Upcoming'}/></section>{edition?<section className={`champion-banner premium-champion-banner${event.level==='Grand Slam'?' slam-champion':''}`}>{championPlayer&&<Portrait player={championPlayer} size="profile"/>}<div><span>{universeYearLabel(universe)} champion{kind==='doubles'||kind==='team'?'s':''}</span><strong>{championText}</strong>{championPlayer&&<small><Flag code={championPlayer.country}/>{championPlayer.rarity} · Fame {(championPlayer.fame||0).toLocaleString()}</small>}<p>{edition.finalScore?`Final: ${edition.finalScore}`:'Tournament completed'}</p></div>{event.level==='Grand Slam'&&<em>GRAND SLAM CHAMPION</em>}</section>:<section className="panel upcoming-edition"><h3>Upcoming edition</h3><p>The draw is generated when simulation reaches week {event.week}. Rankings, surface fit, form, fatigue, health and event eligibility determine the field.</p>{event.year===universe.year&&event.week>=universe.week&&!universe.seasonEnded&&<button className="primary-action" onClick={()=>onSimulate?.(event.week)}>Simulate to {name}</button>}</section>}</>}
...
function Leader({title,rows,onPlayer}){return <section className="panel prestige-panel"><h3>{title}</h3>{rows.slice(0,12).map((row,index)=><button className="leader-row portrait-leader-row" key={row.player.id} onClick={()=>onPlayer(row.player.id)}><span>{index+1}</span><Portrait player={row.player} size={index<3?'card':'compact'}/><Flag code={row.player.country}/><strong>{fullName(row.player)}</strong><b>{row.value}</b></button>)}</section>}
